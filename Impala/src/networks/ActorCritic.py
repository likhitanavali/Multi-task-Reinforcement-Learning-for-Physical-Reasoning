...
class ActorCriticLSTM(nn.Module):
    ...
    def _act_dist(self, logits: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        mu = torch.tanh(logits).to('cuda')  # Mean of the multivariate distribution
        cov = torch.eye(self.n_outputs).to('cuda')  # Covariance matrix (identity for simplicity)
        dist = MultivariateNormal(loc=mu, covariance_matrix=cov)  # Use MultivariateNormal
        action = dist.sample()
    ...
    def forward(
        ...
        x = x.view(seq * batch, self.hidden_size)
        behaviour_actions = behaviour_actions.view(seq * batch, 2)  # Shape for dist
