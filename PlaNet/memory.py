...
class ExperienceReplay():
  def __init__(self, size, symbolic_env, observation_size, action_size, bit_depth, device):
    ...
    self.observations = np.empty((size,1,64,64),dtype=np.uint8)
    # self.observations = np.empty((size,84,84,3),dtype=np.float32)
    # self.actions = np.empty((size, action_size), dtype=np.float32)
    self.actions = np.empty((size,3),dtype=np.float32)
  ...
  def _sample_idx(self, L):
    valid_idx = False
    while not valid_idx:
      # print("full = ",self.full," idx =",self.idx)
