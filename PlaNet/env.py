...
GYM_ENVS = ['Pendulum-v0', 'MountainCarContinuous-v0', 'Ant-v2', 'HalfCheetah-v2', 'Hopper-v2', 'Humanoid-v2', 'HumanoidStandup-v2', 'InvertedDoublePendulum-v2', 'InvertedPendulum-v2', 'Reacher-v2', 'Swimmer-v2', 'Walker2d-v2','CreateLevelCustomPush-v0']
...
def preprocess_observation_(observation, bit_depth):
  ...
  # img_mean = torch.mean(observation, dim=0)

  # # threshold the resulting tensor to get a binary image
  # binary_img = torch.ones_like(img_mean)
  # binary_img[img_mean > 0.4] = 0
  # return binary_img - 0.5
...
def preprocess_observation(observation):
  ...
  # threshold the resulting tensor to get a binary image
  binary_img = torch.ones_like(img_mean)
  binary_img[img_mean > 200] = 0
  return torch.unsqueeze(binary_img - 0.5,dim=0)
...
class GymEnv():
  ...
  def reset(self):
    ...
    if self.symbolic:
      ...
      # render_res = self._env.render(mode='rgb_array')
  ...
  def sample_random_action(self):
    ...
    return torch.FloatTensor(self._env.action_space.sample())
