...
def generate_actions(solve):  
  ...
  if solve:
      ...
      actions[4] = [solver_tools[random.randint(0,len(solver_tools)-1)],solver_x_y[1][0],solver_x_y[1][1]]
  return actions

register_json_str(json_str)
# args.experience_size = 1000
# Initialise training environment and experience replay memory
# env = Env('CreateLevelCustomPush-v0', args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth)
# num_tools = 939
...
env_name_list = ['CreateLevelTwoCustomPush0-v0','CreateLevelTwoCustomPush1-v0','CreateLevelTwoCustomPush2-v0','CreateLevelTwoCustomPush3-v0','CreateLevelTwoCustomPush4-v0']
num_tools=len(tools_list)
# args.seed_episodes = 2000
args.episodes = 50700
args.experience_replay = '/home/soham/USC/Courses/CSCI566_DeepLearning/CREATE/PlaNet/Replay_50000_4_tools_5_envs.pth'
# env = Env(args.env, args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth)
if args.experience_replay is not '' and os.path.exists(args.experience_replay):
  D = torch.load(args.experience_replay)
  metrics['steps'], metrics['episodes'] = [D.steps] * D.episodes, list(range(1, D.episodes + 1))
elif not args.test:
  D = ExperienceReplay(args.experience_size, args.symbolic_env, env1.observation_size, env1.action_size, args.bit_depth, args.device)
  # Initialise dataset D with S random seed episodes
  for s in range(1, args.seed_episodes + 1):
    observation, done, t = env1.reset(), False, 0
    solve = True if random.random() > 0.5 else False
    actions = generate_actions(solve)
    solved = False
    while not done:
      ...
      next_observation, reward, done = env1.step(action)
...
transition_model = TransitionModel(args.belief_size, args.state_size, env1.action_size, args.hidden_size, args.embedding_size, args.activation_function).to(device=args.device)
observation_model = ObservationModel(args.symbolic_env, env1.observation_size, args.belief_size, args.state_size, args.embedding_size, args.activation_function,out_channels=1).to(device=args.device)
...
encoder = Encoder(args.symbolic_env, env1.observation_size, args.embedding_size, args.activation_function,in_channels).to(device=args.device)
...
# planner_1 = MPCPlanner(env.action_size, args.planning_horizon, args.optimisation_iters, args.candidates, args.top_candidates, transition_model, reward_model, env.action_range[0], env.action_range[1])
planner_1 = MPCPlanner(env1.action_size, args.planning_horizon, args.optimisation_iters, args.candidates, args.top_candidates, transition_model, reward_model, -1, 1,num_tools)
...
args.action_noise = 0.2

def update_belief_and_act(args, env, planner_1, transition_model, encoder, belief, posterior_state, action, observation, min_action=-inf, max_action=inf, explore=False):
  ...
  action = planner_1(belief, posterior_state)  # Get action from planner(q(s_t|o≤t,a<t), p)
  if explore:
    discrete_action = action[:,:1] if random.random() > args.action_noise else torch.randint_like(action[:,:1],0,num_tools)
    discrete_action = torch.FloatTensor([[tools_list[int(action[i][0])]] for i in range(len(action))]).to(args.device)
    cont_actions = action[:,1:]
    # cont_actions = cont_actions + args.action_noise * torch.randn_like(cont_actions)  # Add exploration noise ε ~ p(ε) to the action
    action = torch.cat((discrete_action,cont_actions),dim=1)
  else:
    discrete_action = torch.FloatTensor([[tools_list[int(action[i][0])]] for i in range(len(action))]).to(args.device)
    cont_actions = action[:,1:]
    action = torch.cat((discrete_action,cont_actions),dim=1)
...
if args.test:
  ...
  with torch.no_grad():
    ...
    for _ in tqdm(range(args.test_episodes)):
      ...
      for t in pbar:
        belief, posterior_state, action, observation, reward, done = update_belief_and_act(args, env, planner_1, transition_model, encoder, belief, posterior_state, action, observation.to(device=args.device), env.action_range[0], env.action_range[1])
        total_reward += reward
        if args.render:
          env1.render()
        if done:
...
for episode in tqdm(range(metrics['episodes'][-1] + 1, args.episodes + 1), total=args.episodes, initial=metrics['episodes'][-1] + 1):
  ...
  for s in tqdm(range(args.collect_interval)):
    ...
    predicted_next_observation = bottle(observation_model, (beliefs, posterior_states))
  ...
  print("Observation_loss = ",sum(losses[0])/len(losses[0]))
  metrics['reward_loss'].append(losses[1])
  print("Reward_loss = ",sum(losses[1])/len(losses[1]))
  metrics['kl_loss'].append(losses[2])
  print("kl_loss = ",sum(losses[2])/len(losses[2]))
  ...
  with torch.no_grad():
    ...
    pbar = tqdm(range(args.max_episode_length // args.action_repeat))
    for t in pbar:
      belief, posterior_state, action, next_observation, reward, done = update_belief_and_act(args, env, planner_1, transition_model, encoder, belief, posterior_state, action, observation.to(device=args.device), env.action_range[0], env.action_range[1], explore=True)
  ...
  if episode % args.test_interval == 0:
    ...
    env_num = random.randint(1,5)
    test_envs = EnvBatcher(Env, (env_name_list[env_num-1], args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth), {}, args.test_episodes)
    ...
    with torch.no_grad():
      ...
      pbar = tqdm(range(args.max_episode_length // args.action_repeat))
      for t in pbar:
        belief, posterior_state, action, next_observation, reward, done = update_belief_and_act(args, test_envs, planner_1, transition_model, encoder, belief, posterior_state, action, observation.to(device=args.device), env.action_range[0], env.action_range[1])
    ...
    (metrics['test_episodes'], metrics['test_rewards'], 'test_rewards', results_dir)
